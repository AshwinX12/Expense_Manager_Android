...
public class TransactionDao_Impl(
  ...
  public override suspend fun getTotalIncome(startDate: LocalDate, endDate: LocalDate): BigDecimal {
    val _sql: String = """
        |
        |        SELECT COALESCE(SUM(amount), 0) FROM transactions
        |        WHERE type = 'INCOME' AND date BETWEEN ? AND ?
  ...
  public override suspend fun fixNullAccountTransactions(defaultAccountId: Long): Int {
    val _sql: String = "UPDATE transactions SET accountId = ? WHERE accountId IS NULL"
    return performSuspending(__db, false, true) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
        _stmt.bindLong(_argIndex, defaultAccountId)
        _stmt.step()
  ...
  public override suspend fun updateAccountIdForRecurringRule(ruleId: Long, accountId: Long): Int {
    val _sql: String = "UPDATE transactions SET accountId = ? WHERE recurringRuleId = ?"
    return performSuspending(__db, false, true) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
